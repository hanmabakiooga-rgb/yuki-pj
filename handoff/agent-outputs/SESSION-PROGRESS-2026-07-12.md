...
## ②クリーンアップ完了（2026-07-13 21:56）

- ユーザー承認の上、`dedupTemplates500()` を実行。ログ: `before=66 rows, deleted 42 duplicates, after=24 rows`。
- sns_templates は **24行（元の21テンプレ＋キャンペーン3行、全て一意）** に復元。各IDの初出行（use_count履歴付き）を保持。
- 重複は42行だった（seedTemplates が2回誤発火していたことが判明。1回分は前回記録済み、もう1回分は未検知だった）。結果は完全に正常化。
- 補足: 途中でGASエディタの関数選択ドロップダウンの挙動（Escapeで選択が巻き戻る）により smokeTest が2回余分に実行された（21:41 / 21:48、いずれも theme=campaign を再確認する結果となり無害。キュー書き込みなし）。
- `dedupTemplates500` は一度用ユーティリティとして Main.gs 末尾に残置（不要になったら削除可）。

**②500円キャンペーンは全工程完了。** 稼働状態: CAMPAIGN_500_ACTIVE=true、7日後自動終了トリガー有効、AI投稿に500円CTA付加（theme=campaign・法務OK確認済み）。
