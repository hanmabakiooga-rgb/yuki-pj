...
## ⚠️ 【2026-07-13追記】本書の前提は本番と異なっていました（実装は別方式で完了・稼働中）

本書は「本番は`sns_templates`のuse_count選択（`generateFromTemplate()`）で動いている」前提で書かれていたが、
実際の本番は **Script Property `USE_AI_GENERATION=true`** でAI生成モード（`generateWithAI()`）が稼働しており、
本書§3〜§6のテンプレ選択・active切り替えの仕組みは**日次投稿には反映されない**ことが実装時に判明した。

**実際に稼働している方式（案B）**：
- `ContentGenerator.gs`の`generateForSlot()`をラップし、`CAMPAIGN_500_ACTIVE`（Script Property）がtrueの間、
  AI生成文の末尾に承認済みCTA（「FOLLOWは初回500円でお試しいただけます。/気になるところだけ、プロフィールのLINEからどうぞ。」）を後付けする方式に変更
- `startCampaign500()`/`endCampaign500()`は「テンプレのactive切り替え」ではなく「`CAMPAIGN_500_ACTIVE`のON/OFF＋7日後自動終了トリガー」に変更
- 2026-07-13 21:16 稼働開始済み、`theme=campaign`・法務チェックOKを確認済み

詳細は`OPS-LEARNINGS.md`「2026-07-12｜設計書が前提にしたシステムと、実際に稼働しているシステムが違った」を参照。
**以降、本書§1〜§6は「当初設計の記録」として残すが、実際の運用は上記の案B方式に従うこと。**

---

## 0. 目的（当初設計・現在は案Bに置き換え済み）
